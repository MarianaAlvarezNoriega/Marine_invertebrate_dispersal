...
# Estimate the phylogenetic signal (lambda)
hyp <- "sd_Genus__Intercept^2 / (sd_Genus__Intercept^2 + sigma^2) = 0"
...
#write.table(Plank_post, 'PLD_posterior.csv', sep=',', row.n=F)
...
Q2a<-ggplot()+geom_raster(data=new, aes(x=Longitude, y=Latitude, fill=fit))+
  ...
  geom_count(data=datPD[datPD$DevelopmentalMode=='P',], aes(x=Longitude,y=Latitude),  col='grey', alpha=0.4, show.legend = T)+
  ...
  xlab('')+ylab('')+theme(legend.direction = "vertical", legend.box = "horizontal")
Q2a
...
Q2b<-ggplot()+geom_raster(data=new, aes(x=Longitude, y=Latitude, fill=fit))+
  geom_polygon(data=world, aes(x=long, y=lat, group=group))+theme_classic()+
  scale_fill_gradient(low='#FFCCCC', high='#990000', breaks=c(log10(10),log10(8), log10(6),log10(4)),
                      labels=c('10','8', '6', '4'))+
  geom_count(data=datPD[datPD$DevelopmentalMode=='L',], aes(x=Longitude,y=Latitude),  col='grey', alpha=0.4, show.legend = T)+
  guides(fill=guide_legend(title='PD \n(days)'))+
  theme(axis.line = element_line(size = 1), axis.ticks.length = unit(0.25, 'cm'),
        text=element_text(size=18), plot.title=element_text(hjust=0.5))+

...
# Get GAM predictions 
mod<-readRDS('gam_predictions.rds')
df<-data.frame('latitude'=seq(-55,55,by=1))


df$fit<-apply(mod, 1, function(x) median(x,na.rm=T))
df$lwr<-apply(mod, 1, function(x) quantile(x, 0.025,na.rm=T))
df$upr<-apply(mod, 1, function(x) quantile(x, 0.975,na.rm=T))
...
Fig4b-ggplot()+
  ...
  scale_y_continuous(breaks=c(
                              log10(50),
                              log10(100), log10(150),log10(200),log10(250),log10(300),log10(350)),
                     labels=c(50,100,150,200,250,300,350), limits=c(log10(50), log10(350)))+
  scale_x_continuous(breaks=c(-80, -60, -40,-20,0,20,40,60,80), 
                     labels=c('80°S','60°S',  '40°S', '20°S','0°', '20°N', '40°N',  '60°N','80°N'))+
  theme_classic()+
  ylab('Dispersal (km)')+

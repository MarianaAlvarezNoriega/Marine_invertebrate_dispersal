...
model<-brm(Planktotrophic~absLatitude+absLatitude:Hemisphere,
          family=binomial(),
          data = datPD,seed=17,
          control = list(max_treedepth = 17, adapt_delta=0.999), chains=3,thin = 5, iter=30000, 
          warmup=5000, file='Planktotrophy_noPhylogeny')
...
#write.table(Plank_post, 'feeding_posterior_no_phylo.csv', sep=',', row.n=F)
...
FigS14<-ggplot()+geom_raster(data=new, aes(x=Longitude, y=Latitude, fill=fit))+
  geom_polygon(data=world, aes(x=long, y=lat, group=group))+theme_classic()+
  scale_fill_gradient(high='#990000', low='#FFCCCC', breaks=c(0.8,0.7,0.6,0.5,0.4))+
  geom_count(data=datPD, aes(x=Longitude,y=Latitude, shape=DevelopmentalMode),  col='white', alpha=0.4, show.legend = F)+

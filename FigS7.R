...
# Fit model
prior1<-get_prior(log10(EggSize)~DevelopmentalMode:absLatitude:Hemisphere+absLatitude*DevelopmentalMode+(1|Genus),
                  family=gaussian(),data=datPD)

model<-brm(log10(EggSize)~DevelopmentalMode:absLatitude:Hemisphere+absLatitude*DevelopmentalMode+(1|Genus),
          data = datPD,seed=17,
...
#write.table(egg_post, 'egg_posterior.csv', sep=',', row.n=F)
...
P<-ggplot()+geom_raster(data=new, aes(x=Longitude, y=Latitude, fill=fit))+
  ...
  scale_fill_gradient( high='#990000', low='#FFCCCC',
                       breaks=c(log10(114),log10(113),log10(112),log10(111), log10(110)),
                       labels=c(114,113,112,111,110))+
